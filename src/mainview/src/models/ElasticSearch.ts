...
export default class ElasticSearch extends Client {
  async indices(): Promise<IndicesData> {
    const records = await this.request<ElasticSearchIndicesResponse>(
      `https://${this.project.host}:${this.project.port}/_cat/indices?format=json`,
    );
    ...
    return records
      .filter(record => !record.index.startsWith('.'))
      .map(record => ({
        id: record.uuid,
        name: record.index,
  ...
  async documents(index: string, page: number, search?: string): Promise<DocumentsData> {
    ...
    const records = await this.request<ElasticSearchSearchResponse>(
      `https://${this.project.host}:${this.project.port}/${index}/_search?format=json&size=${pageSize}&from=${pageOffset}`,
      body,
    );
    if (!records) return [];
  ...
  async mappings(index: string): Promise<any> {
    return await this.request(
      `https://${this.project.host}:${this.project.port}/${index}/_mapping?format=json`,
    );
  }

  async settings(index: string): Promise<any> {
    return await this.request(
      `https://${this.project.host}:${this.project.port}/${index}/_settings?format=json`,
    );
  }
}

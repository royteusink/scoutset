<template>
  <div class="h-screen flex flex-col justify-between p-4">
    <header class="flex-none flex justify-between border-b border-gray-200 pb-4 mb-6">
      ...
      <RouterLink
        to="/"
        class="bg-gray-100 rounded-full overflow-clip flex-none inline-flex justify-center items-center w-10 h-10 focus:bg-gray-200 hover:bg-gray-200 outline-hidden"
      >
    ...
    <section class="overflow-auto flex-1">
      ...
      <ul class="flex flex-col gap-4">
        <li
          v-for="project in availableProjects.values()"
          :key="project.id"
        ...
        >
          <button
            ...
            class="block text-left outline-hidden leading-none overflow-clip max-w-full"
          >
            <div class="truncate w-full group-focus:underline group-hover:underline">
              {{ project.name }}
            </div>
          ...
          <button
            ...
            class="invisible group-hover:visible group-focus-within:visible bg-gray-100 rounded-full overflow-clip flex-none inline-flex justify-center items-center w-5 h-5 focus:bg-gray-200 hover:bg-gray-200 outline-hidden"
          >
    ...
    <section class="flex-none pt-4">
      ...
      <form @submit.prevent="handleSubmit" class="bg-gray-100 p-4 flex flex-wrap gap-4">
        ...
        <div>
          ...
          <input
            type="text"
            :placeholder="host ? host : 'name'"
            v-model="name"
            class="p-2 border border-gray-200"
          />
        </div>
        <button
          class="self-end bg-blue-600 px-4 text-sm text-white rounded-sm shadow-sm outline-hidden focus:bg-blue-800 hover:bg-blue-800 flex-none h-10"
        >
          Create project
        </button>
      </form>
...
const { createProject, openProject, activateProject, availableProjects, deleteProject } =
  useProjects();

    ...
    <div class="table min-w-full">
      <div class="table-header-group sticky top-0">
        <div class="table-row from-white to-gray-50 bg-gradient-to-b">
          <div v-for="(column, coldex) in columns" :key="column" class="table-cell py-1 pl-2 pr-2 text-gray-600 border-b border-gray-200 border-r last:border-r-0" @click="emit('clickColumn', column)">
            {{ column }}
          </div>
        </div>
      </div>
      ...
      <div class="table-row-group">
        <div v-for="row in rows" :key="row.id" @click="emit('clickRow', row); selectedRow = row.id" :class="{
          ...
          <div v-for="column in columns" :key="`${row.id}_${column}`" :class="{ 'table-cell py-1 pl-2 pr-2 truncate max-w-64': true }">
